...
export default function OrderCard({ order, isSelected, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`relative flex rounded-3xl border-2 overflow-hidden transition-all cursor-pointer min-h-44 w-full
        bg-softwhite dark:bg-[#1e1c1a]
        ${
          isSelected
            ? "border-charcoal dark:border-white/30 shadow-xl dark:shadow-black/40"
      ...
      <div className="flex-1 px-3 py-4 flex flex-col">
        <div className="flex justify-between items-start mb-2">
          <div className="flex flex-row gap-2">
            <div
              className={`p-2 px-2.5 rounded-xl ${type.color} text-white shadow-lg shrink-0 flex items-center`}
            >
              <Icon size={16} />
            </div>
            <div>
              <h3 className="font-serif font-black text-charcoal dark:text-stone-100 text-base">
          ...
          <div className="flex flex-col items-end">
            <p className="text-lg font-serif font-black text-charcoal dark:text-stone-100">
              ${order.total}
            </p>
        ...
        <div className="flex-1">
          <div className="flex flex-wrap gap-1 mb-2 overflow-hidden max-h-16">
            {order.order_items?.map((item) => (
              <span
                key={item.id}
                className="px-1.5 py-0.5 bg-cream/60 dark:bg-white/5 rounded text-[10px] font-bold text-charcoal/55 dark:text-white/40"
              >
        ...
        {/* Footer */}
        <div className="mt-auto flex justify-between items-center border-t border-cream dark:border-white/5 pt-2 text-[10px] font-black text-charcoal/40 dark:text-white/30 uppercase">
